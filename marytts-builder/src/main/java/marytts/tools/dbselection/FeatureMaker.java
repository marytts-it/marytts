    ...
    protected static Document phonemiseText(String textString, int id) throws Exception{
		...
		protected static Vector<String> splitIntoSentences(String text, int id, boolean test)throws Exception{
			...
			for (int j=0;j<sentences.getLength();j++){
				...
				// check and fix the spaces on double quotes punctuation in sentences
				try{
				sentence = respaceDoubleQuotes(sentence);
				 } catch(Exception e){
		                System.out.println("Exception on rebaseDoubleQuotes: "+e+ " in sentence <" + sentence.toString() + ">");
        ...
        /*
         *  Fixes the spaces on balanced double quotes
         *  Hypothesis: the  inputSentence is generated by collect token, so in the form: 
         *  this is a " test sentence ".
         *  return StringBuilder corrected sentence 
         */
        private static StringBuilder respaceDoubleQuotes(StringBuilder inputSentence) {
        	if (inputSentence ==null)
        		return null;
    	    boolean open_double_quote = false;
    	 	boolean this_char_is_a_double_quote = false;
    	 	boolean remove_space_on_the_next_char = false;
    	 	
    	 	String sentenceString = inputSentence.toString();
    	 	
    	 	//System.out.println(sentence.toString());
    	 	
    	 	StringBuilder outputSentence = null;
    	 	if (sentenceString.length() > 0)
    	 	{
    	 	 outputSentence = new StringBuilder();
        	 for (int i = 0; i < sentenceString.length(); i++){
        	    char c = sentenceString.charAt(i);        
				//Process char
        	    if (c == '"' )
        	    {
        	    	this_char_is_a_double_quote = true;
        	    	open_double_quote = !open_double_quote;
        	    }
        	    
        		
        	    if (this_char_is_a_double_quote)
        	    {
        	    	if ((outputSentence.length()>0) && !(outputSentence.charAt(outputSentence.length()-1) == ' ')) 
        	    		// the previous char (if exists) have to be a space char
        	    		 throw new IllegalStateException("previous char on closing double quote is not a space");	
        	    	if (open_double_quote )
        	    	{
        	    		//remove the space on the next char
        	    		remove_space_on_the_next_char = true;
        	    	    outputSentence.append(c);
        	    	}
        	    	else
        	    	{
        	    		// put " in plaece of the space
        	    		outputSentence.replace(outputSentence.length()-1, outputSentence.length(), Character.toString(c));	
        	    	}
        	    }
        	    else {
        	    	if (remove_space_on_the_next_char)
            		{
            			remove_space_on_the_next_char = false;
            	    	if (! (c == ' '))
            	    		// the current char  must be a space 
            	    		 throw new IllegalStateException("current char on opening double quote is not a space");	
            		} else
            		{
            		// all other cases
        	    	outputSentence.append(c);
        	    ...
        	    this_char_is_a_double_quote = false;
        	 }
        	}
